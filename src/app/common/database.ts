...
import { db, storage } from "@/infrastracture/firebase/client";
...
export const getProducts = async ({
  ...
  return Promise.all(
    querySnapshot.docs.map((doc) => convertProduct(doc.data(), doc.id)),
  );
};
...
export const getBlogEntries = async ({
  ...
  const querySnapshot = await getDocs(query(colRef, ...queryConstraints));
  return Promise.all(
    querySnapshot.docs.map((doc) => convertBlogEntry(doc.data(), doc.id)),
  );
};

export const getBlogEntry = async (
  id: string,
): Promise<BlogEntryWithId | null> => {
  ...
  const content = await Promise.all(
    (data.content ?? []).map(async (content: any) => {
      if (content.type === "images") {
        ...
        throw new Error(
          "Unexpected content type in blog entry: " + content.type,
        );
      }
    }),
  );
